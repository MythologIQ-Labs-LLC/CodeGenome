...
## [Unreleased] — Phase 4: product differentiation (first tranche)

### Added
- **W3C PROV-JSON export** (`graph/prov.rs`, `codegenome export-prov`): nodes and edge-claims become `prov:Entity`s generated by per-actor `prov:Activity`s and attributed to `prov:Agent`s (PROV-AGENT pattern). Inferred and belief-derived claims carry `cg:aiGenerated: true`, keeping LLM-derived facts distinguishable from compiler-grade observations.
- **In-toto attestation of index snapshots** (`store/attest.rs`, `codegenome attest` / `--verify`): Statement v1 with BLAKE3 digests over every store artifact plus index metadata as predicate; verification reports tampered or missing artifacts. First step of backlog W4/W5 — Sigstore signing is a release-pipeline follow-up (`cosign attest`). This is the substrate-level answer to the memory-poisoning threat model.
- **Markdown repo-map projection** (`graph/repo_map.rs`, `codegenome map`): file-style projection of the graph (signature lines + call-edge fan-in/fan-out) for memory-tool-native agents.
- **Claim-level auditability in MCP responses**: `codegenome_context` reports full node addresses and per-edge claims (endpoints, relation, confidence, evidence count) instead of a bare edge tally; `codegenome_impact` items carry re-queryable addresses.
...
- Deferred: bi-temporal edge validity (schema change), grep-baseline benchmarks with token accounting, MIF/AMP export adapter, MCP structured output/tasks extension, edition 2024.
